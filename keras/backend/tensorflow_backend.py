...
def dot(x, y):
    ...
    if ndim(x) is not None and (ndim(x) > 2 or ndim(y) > 2):
        x_shape = []
        for i, s in zip(int_shape(x), tf.unstack(tf.shape(input=x))):
            if i is not None:
                x_shape.append(i)
            else:
                x_shape.append(s)
        ...
        y_shape = []
        for i, s in zip(int_shape(y), tf.unstack(tf.shape(input=y))):
            if i is not None:
                y_shape.append(i)
            else:
                y_shape.append(s)
...
def prod(x, axis=None, keepdims=False):
    ...
    """
    return tf.reduce_prod(input_tensor=x, axis=axis, keepdims=keepdims)
...
def var(x, axis=None, keepdims=False):
    ...
    return tf.reduce_mean(input_tensor=devs_squared,
                          axis=axis,
                          keepdims=keepdims)
...
def any(x, axis=None, keepdims=False):
    ...
    """
    x = tf.cast(x, tf.bool)
    return tf.reduce_any(input_tensor=x, axis=axis, keepdims=keepdims)
...
def all(x, axis=None, keepdims=False):
    ...
    """
    x = tf.cast(x, tf.bool)
    return tf.reduce_all(input_tensor=x, axis=axis, keepdims=keepdims)
...
def argmin(x, axis=-1):
    ...
    # Returns
        A tensor.
    """
    return tf.argmin(input=x, axis=axis)
...
def logsumexp(x, axis=None, keepdims=False):
    ...
    # Returns
        The reduced tensor.
    """
    return tf.reduce_logsumexp(input_tensor=x, axis=axis, keepdims=keepdims)
...
def _regular_normalize_batch_in_training(x, gamma, beta,
    ...
    """
    mean, var = tf.nn.moments(x=x, axes=reduction_axes,
...
def _broadcast_normalize_batch_in_training(x, gamma, beta,
    ...
    for axis in range(ndim(x)):
        if axis in reduction_axes:
            ...
            target_shape.append(tf.shape(input=x)[axis])
...
def permute_dimensions(x, pattern):
    ...
    # Returns
        A tensor.
    """
    return tf.transpose(a=x, perm=pattern)
...
def resize_images(x, height_factor, width_factor, data_format):
    ...
    if data_format == 'channels_first':
        ...
        original_shape = int_shape(x)
        new_shape = tf.shape(input=x)[1:3]
        new_shape *= tf.constant(np.array([height_factor, width_factor]).astype('int32'))
        x = tf.image.resize(x, new_shape, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
...
def print_tensor(x, message=''):
    ...
    """
    return tf.compat.v1.Print(x, [x], message)
...
def gradients(loss, variables):
    ...
    # Returns
        A gradients tensor.
    """
    return tf.gradients(ys=loss, xs=variables)
...
def rnn(step_function, inputs, initial_states,
    ...
    if mask is not None:
        ...
        mask = tf.transpose(a=mask, perm=axes)
        ...
        if mask is not None:
            ...
            for inp, mask_t in zip(input_list, mask_list):
                ...
                tiled_mask_t = tf.tile(mask_t,
                                       tf.stack([1, tf.shape(input=output)[1]]))
                ...
                for state, new_state in zip(states, new_states):
                    ...
                    tiled_mask_t = tf.tile(mask_t,
                                           tf.stack([1, tf.shape(input=new_state)[1]]))
...
def switch(condition, then_expression, else_expression):
    ...
    if not cond_ndim:
        ...
        x = tf.cond(pred=condition,
                    true_fn=then_expression_fn,
                    false_fn=else_expression_fn)
    else:
        # tf.where needs its condition tensor
        # to be the same shape as its two
        # result tensors
        ...
        if cond_ndim > 1:
            ...
            cond_shape = tf.concat([tf.shape(input=condition), [1] * ndim_diff], axis=0)
            condition = tf.reshape(condition, cond_shape)
            expr_shape = tf.shape(input=then_expression)
...
def categorical_crossentropy(target, output, from_logits=False):
    ...
    if not from_logits:
        ...
        output /= tf.reduce_sum(input_tensor=output,
                                axis=len(output.get_shape()) - 1,
                                keepdims=True)
        # manual computation of crossentropy
        _epsilon = _to_tensor(epsilon(), output.dtype.base_dtype)
        output = tf.clip_by_value(output, _epsilon, 1. - _epsilon)
        return - tf.reduce_sum(input_tensor=target * tf.math.log(output),
                               axis=len(output.get_shape()) - 1)
    else:
        return tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(target),
                                                       logits=output)
...
def sparse_categorical_crossentropy(target, output, from_logits=False):
    ...
    if len(output_shape) >= 3:
        ...
        return tf.reshape(res, tf.shape(input=output)[:-1])
    else:
        return res
...
def dropout(x, level, noise_shape=None, seed=None):
    ...
    return tf.nn.dropout(x * 1., 1 - (retain_prob), noise_shape, seed=seed)
...
def in_top_k(predictions, targets, k):
    ...
    """
    return tf.nn.in_top_k(predictions=predictions, targets=targets, k=k)
...
def _preprocess_conv1d_input(x, data_format):
    ...
    if data_format == 'channels_first':
        if not _has_nchw_support():
            x = tf.transpose(a=x, perm=(0, 2, 1))  # NCW -> NWC
        else:
            tf_data_format = 'NCHW'
...
def _preprocess_conv2d_input(x, data_format):
    ...
    if data_format == 'channels_first':
        if not _has_nchw_support():
            x = tf.transpose(a=x, perm=(0, 2, 3, 1))  # NCHW -> NHWC
        else:
            tf_data_format = 'NCHW'
    return x, tf_data_format
...
def conv2d(x, kernel, strides=(1, 1), padding='valid',
    ...
    if data_format == 'channels_first' and tf_data_format == 'NHWC':
        x = tf.transpose(a=x, perm=(0, 3, 1, 2))  # NHWC -> NCHW
    return x
...
def conv2d_transpose(x, kernel, output_shape, strides=(1, 1),
    ...
    if output_shape[0] is None:
        output_shape = (tf.shape(input=x)[0],) + tuple(output_shape[1:])
...
def separable_conv1d(x, depthwise_kernel, pointwise_kernel, strides=1,
    ...
    x = tf.nn.separable_conv2d(input=x, depthwise_filter=depthwise_kernel, pointwise_filter=pointwise_kernel,
                               strides=strides,
                               padding=padding,
                               dilations=dilation_rate,
                               data_format=tf_data_format)
    ...
    if data_format == 'channels_first' and tf_data_format == 'NHWC':
        x = tf.transpose(a=x, perm=(0, 2, 1))  # NWC -> NCW
...
def depthwise_conv2d(x, depthwise_kernel, strides=(1, 1), padding='valid',
    ...
    x = tf.nn.depthwise_conv2d(input=x, filter=depthwise_kernel,
                               strides=strides,
                               padding=padding,
                               dilations=dilation_rate,
                               data_format=tf_data_format)
...
def pool2d(x, pool_size, strides=(1, 1),
    ...
    if pool_mode == 'max':
        x = tf.nn.max_pool2d(input=x, ksize=pool_size, strides=strides,
                           padding=padding,
                           data_format=tf_data_format)
    elif pool_mode == 'avg':
        x = tf.nn.avg_pool2d(input=x, ksize=pool_size, strides=strides,
                           padding=padding,
                           data_format=tf_data_format)
    else:
...
def ctc_label_dense_to_sparse(labels, label_lengths):
    ...
    """
    label_shape = tf.shape(input=labels)
    ...
    label_array = tf.reshape(tf.tile(tf.range(label_shape[1]), num_batches_tns),
                             label_shape)
    label_ind = tf.boolean_mask(tensor=label_array, mask=dense_mask)

    batch_array = tf.transpose(a=tf.reshape(tf.tile(tf.range(label_shape[0]),
                                                  max_num_labels_tns), reverse(label_shape, 0)))
    batch_ind = tf.boolean_mask(tensor=batch_array, mask=dense_mask)
    indices = tf.transpose(a=tf.reshape(concatenate([batch_ind, label_ind], axis=0), [2, -1]))

    vals_sparse = tf.gather_nd(labels, indices)

    return tf.SparseTensor(tf.cast(indices, dtype=tf.int64), vals_sparse, tf.cast(label_shape, dtype=tf.int64))
...
def ctc_batch_cost(y_true, y_pred, input_length, label_length):
    ...
    y_pred = tf.math.log(tf.transpose(a=y_pred, perm=[1, 0, 2]) + epsilon())
